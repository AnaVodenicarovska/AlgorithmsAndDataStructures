...
class ImeLek implements Comparable<ImeLek>{
	...
	public ImeLek(String imeLek, int pozneg, int cena, int brLekovi) {
		super();
		this.imeLek = imeLek;
		this.pozneg = pozneg;
		this.cena = cena;
...
public class HashApteka {
	...
	public static void main(String[] args) throws IOException {
		...
		for(i=0;i<N;i++) {
			...
			il=new ImeLek(imeLek,pozneg,cena,brLekovi);
			tabela.insert(il,il);
		}
		s=br.readLine();
		while(!s.equals("KRAJ")) {
			...
			il=new ImeLek(imeLek,0,0,0);
			tmp=tabela.search(il);
			if(tmp==null)
				System.out.println("Nema takov lek");
			else {
				ImeLek najden=tmp.element.value;
